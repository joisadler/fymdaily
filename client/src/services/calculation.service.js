...
const getNumbers = (user, eatenFoods) => {
  ...
  const bodyFatPercentage =
    gender === 'male' // US Navy method to calculate body fat percentage
      ? 495 /
          (1.0324 -
  ...
  const dailyCarbNeed =
    (dailyCalorieNeed -
      dailyProteinNeed * caloriesPerGramOfProtein -
      dailyFatNeed * caloriesPerGramOfFat) /
    caloriesPerGramOfCarb;

  const currentCalories = eatenFoods.reduce(
    (acc, current) => acc + (current.calories * current.weight) / 100,
    0
  );
  const currentProteins = eatenFoods.reduce(
    (acc, current) => acc + (current.proteins * current.weight) / 100,
    0
  );
  const currentFats = eatenFoods.reduce(
    (acc, current) => acc + (current.fats * current.weight) / 100,
    0
  );
  const currentCarbs = eatenFoods.reduce(
    (acc, current) => acc + (current.carbs * current.weight) / 100,
    0
  );

  const currentCaloriesRemainder = dailyCalorieNeed - currentCalories;

  const currentPercentOfDailyCalorieNeed =
    (currentCalories / dailyCalorieNeed) * 100;
  const currentPercentOfDailyProteinNeed =
    (currentProteins / dailyProteinNeed) * 100;
  const currentPercentOfDailyFatNeed = (currentFats / dailyFatNeed) * 100;
  const currentPercentOfDailyCarbNeed = (currentCarbs / dailyCarbNeed) * 100;
  ...
  const percentageOfProteinsInDailyCalorieNeed =
    (currentProteinsToCalories / dailyCalorieNeed) * 100;
  const percentageOfFatsInDailyCalorieNeed =
    (currentFatsToCalories / dailyCalorieNeed) * 100;
  const percentageOfCarbsInDailyCalorieNeed =
    (currentCarbsToCalories / dailyCalorieNeed) * 100;
  const percentageOfEmptyCaloriesInDailyCalorieNeed =
    ((currentCalories -
  ...
  const percentageOfCaloriesRemainderInDailyCalorieNeed =
    100 -
  ...
  return {
    ...
    currentPercentOfDailyCalorieNeed: Math.round(
      currentPercentOfDailyCalorieNeed
    ),
    currentPercentOfDailyProteinNeed: Math.round(
      currentPercentOfDailyProteinNeed
    ),
    ...
    percentageOfProteinsInDailyCalorieNeed: Math.round(
      percentageOfProteinsInDailyCalorieNeed
    ),
    percentageOfFatsInDailyCalorieNeed: Math.round(
      percentageOfFatsInDailyCalorieNeed
    ),
    percentageOfCarbsInDailyCalorieNeed: Math.round(
      percentageOfCarbsInDailyCalorieNeed
    ),
    percentageOfCaloriesRemainderInDailyCalorieNeed: Math.round(
      percentageOfCaloriesRemainderInDailyCalorieNeed
    ),
    percentageOfEmptyCaloriesInDailyCalorieNeed: Math.round(
      percentageOfEmptyCaloriesInDailyCalorieNeed
    ),
