...
export async function getFoods(name, showOnlyFoodsCreatedByUser) {
  const foods = await httpService.get(
    `food?name=${name}&showOnlyFoodsCreatedByUser=${showOnlyFoodsCreatedByUser}`
  );
  return foods;
}
...
export async function getCustomFoods(name, showOnlyFoodsCreatedByUser) {
  const foods = await httpService.get(
    `food?name=${name}&custom=true&showOnlyFoodsCreatedByUser=${showOnlyFoodsCreatedByUser}`
  );
  return foods;
}
